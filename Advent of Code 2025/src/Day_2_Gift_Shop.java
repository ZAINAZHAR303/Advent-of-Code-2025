...
public class Day_2_Gift_Shop {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Sum of invalid IDs: " + findInvalidSum(input));
    }

    public static long findInvalidSum(String input) {
        long totalSum = 0;
        for (String rangeStr : input.split(",")) {
            ...
            for (int halfLen = 1; halfLen <= 9; halfLen++) {
                int fullLen = halfLen * 2;
                long pow = (long) Math.pow(10, halfLen);

                long minX = (long) Math.ceil((double) start / (pow + 1));
                long maxX = (long) Math.floor((double) end / (pow + 1));
                long minXDigits = (long) Math.pow(10, halfLen - 1);
                long maxXDigits = pow - 1;

                long actualMinX = Math.max(minX, minXDigits);
                long actualMaxX = Math.min(maxX, maxXDigits);

                if (actualMinX <= actualMaxX) {
                    long count = actualMaxX - actualMinX + 1;
                    long first = actualMinX * (pow + 1);
                    long last = actualMaxX * (pow + 1);
                    totalSum += count * (first + last) / 2;
                }
            }
        }
        return totalSum;
